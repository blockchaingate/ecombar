...
export class ProductAddComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (this.uuid) {
            ...
            this.productServ.getProductInfo(this.uuid).subscribe(
                (res: any) => {
                    if (res && res.status == 200 && res.data) {
                        ...
                        if (product.titleSet) {
                            this.title = product.titleSet.en;
                            this.titleChinese = product.titleSet.sc;
                            this.titleTradition = product.titleSet.tc;
                        }
                        if (product.subtitleSet) {
                            this.subtitle = product.subtitleSet.en;
                            this.subtitleChinese = product.subtitleSet.sc;
                            this.subtitleTradition = product.subtitleSet.tc;
                        }
                        if (product.descSet) {
                            this.description = product.descSet.en;
                            this.descriptionChinese = product.descSet.sc;
                            this.descriptionTradition = product.descSet.tc;
                        }
                        ...
                        if (product.flavorSet) {
                            if(product.flavorSet.en) {
                                this.colors = product.flavorSet.en;
                            }
                            if(product.flavorSet.sc) {
                                this.colorsChinese = product.flavorSet.sc;
                            }
                            if(product.flavorSet.tc) {
                                this.colorsTradition = product.flavorSet.tc;
                            }
                        }
                        if(product.sizesSet) {
                            if(product.sizesSet.en) {
                                this.sizes = product.sizesSet.en;
                            }
                            if(product.sizesSet.sc) {
                                this.sizesChinese = product.sizesSet.sc;
                            }

                            if(product.sizesSet.tc) {
                                this.sizesTradition = product.sizesSet.tc;
                            }
                        }
    ...
    saveProduct() {
        let data: any = {
            ...
            subtitleSc: this.subtitleChinese,
            subtitleTc: this.subtitleTradition,
