...
export class ProductComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        //     }
        // );
        this.productServ.getProductInfo( this.id ).subscribe(
            (res: any) => {
                if (res && res.status == 200 && res.data) {
                    ...
                    this.colors = [];
                    if (this.product.flavorSet && this.product.flavorSet.en) {  // 开始拼凑
                        const enSize = this.product.flavorSet.en.length;
                        const scSize = this.product.flavorSet.sc.length;
                        const tcSize = this.product.flavorSet.tc.length;
                        for (let i = 0; i < enSize; i ++) {
                            this.colors.push( {
                                en : i < enSize ? this.product.flavorSet.en[i] : '',
                                sc : i < scSize ? this.product.flavorSet.sc[i] : this.product.flavorSet.en[i],
                                tc : i < tcSize ? this.product.flavorSet.tc[i] : this.product.flavorSet.en[i],
                            } );
                        }
                    }
                    this.sizes = [];
                    if (this.product.sizesSet && this.product.sizesSet.en) {  // 开始拼凑
                        const enSize = this.product.sizesSet.en.length;
                        const scSize = this.product.sizesSet.sc.length;
                        const tcSize = this.product.sizesSet.tc.length;
                        for (let i = 0; i < enSize; i ++) {
                            this.sizes.push( {
                                en : i < enSize ? this.product.sizesSet.en[i] : '',
                                sc : i < scSize ? this.product.sizesSet.sc[i] : this.product.sizesSet.en[i],
                                tc : i < tcSize ? this.product.sizesSet.tc[i] : this.product.sizesSet.en[i],
                            } );
                        }
                    }
                }
    ...
    addToCart( product: any, quantity: number ) {
        ...
        console.log('addToCart gogo2');
        console.log('product==', product);
        const cartItem: CartItem = {  // 取消 rebateRate, lockedDays, storeId, currency
            productId: product.id,  // product._id
            objectId: product.objectId,
            title: product.title,  // this.translateServ.transField(product.title),
            price: product.price,
            rebateRate: 0,  // product.rebateRate ? product.rebateRate : this.store.rebateRate,
            taxRate: product.taxRate ? product.taxRate : 0,  // product.taxRate ? product.taxRate : this.store.taxRate
            storeId: null,  // this.storeId,
            size: this.size,  // this.lang == 'en' ? this.size : this.sizeChinese,
            color: this.color,  // this.lang == 'en' ? this.color : this.colorChinese,
            lockedDays: 0,  // product.lockedDays ? product.lockedDays : this.store.lockedDays,
            currency: null,  // product.currency,
